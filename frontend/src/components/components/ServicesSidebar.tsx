...
export const ServicesSidebar: React.FC<ServicesSidebarProps> = ({
  ...
  return (
    <aside id="services" className="w-full bg-[#1E3A5F]/20 border border-[#1E3A5F] rounded-[3px] p-4 md:p-5">
      ...
      {/* Vertical Rows List (Numbers 01-09 removed, all marked as 即將登場) */}
        ...
        {SIDEBAR_PRICE_LIST.map((service) => {
          ...
          return (
            <div
              ...
              className={`group py-3 px-2.5 rounded-[2px] cursor-pointer transition-all duration-150 flex items-center justify-between gap-3 ${
                isSelected
                  ? 'bg-[#1E3A5F] text-[#F4EFEA]'
                  : 'hover:bg-[#F4EFEA]/10 text-[#F4EFEA]'
              }`}
            >
              {/* Left: Title + Turnaround (No 01-09 numbers) */}
              <div className="min-w-0">
                <div className="font-serif text-sm font-medium tracking-wide truncate group-hover:text-[#D97706] transition-colors">
                  {service.title}
                </div>
                <div className="text-[11px] text-[#A4B3C6] font-sans truncate">
                  {service.turnaround}
                </div>
              </div>
